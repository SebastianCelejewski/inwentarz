...
public class BookDao implements IBookDao {
	...
	result.append("Tytu≥: " + bookData.getTytul() + "\n");
	...
	if (bookData.getWartosc() != null) {
	    result.append("WartoúÊ: " + cf.format(bookData.getWartosc()) + "\n");
	}

	if (bookData.getDataWlaczenia() != null) {
	    result.append("Data w≥πczenia: " + df.format(bookData.getDataWlaczenia()) + "\n");
	}

	result.append("èrÛd≥o: " + bookData.getZrodloId() + "\n");
	...
	try {
		...
		throw new InwentarzException("Skontrum nie zosta≥o rozpoczÍte. Rozpocznij nowe skontrum aby weryfikowaÊ posiadane ksiπøki.");
	    }
		...
		throw new InwentarzException("Ksiπøka by≥a juø weryfikowana w trakcie bieøπcego skontrum w dniu " + dtf.format(dataWeryfikacji) + ".");
	    }
	...
	try {
	    if (!scontrumDao.isScontrumOpen()) {
		throw new RuntimeException("Scontrum nie jest rozpoczÍte");
	    }
	    int verificationId = scontrumDao.getOpenScontrumId();
	    Statement statement = connectionFactory.getConnection().createStatement();
	    ...
	    throw new RuntimeException("Brak danych na temat weryfikacji ksiπøki " + bookId);
	} catch (Exception ex) {
