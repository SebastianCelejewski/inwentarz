...
public class ScontrumListPanel extends JPanel implements IScontrumListEventListener {
    ...
    public void finishCurrentScontrum() {
        if (scontrumDao.isScontrumOpen()) {
            ...
            int result = JOptionPane.showConfirmDialog(this, "Nie wszystkie książki zostały zweryfikowane - pozostało " + liczbaNiezweryfikowanychKsiazek + " książek.\nCzy mimo to chcesz zakończyć skontrum?");
    ...
    public void startNewScontrum() {
        if (scontrumDao.isScontrumOpen()) {
            JOptionPane.showMessageDialog(this, "Skontrum jest rozpoczęte. Nie możzna rozpocząć nowego skontrum przed zakończeniem poprzedniego.", "Błąd", JOptionPane.INFORMATION_MESSAGE);
            return;
        }
